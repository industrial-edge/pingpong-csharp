...
- [Create configuration for the application](#create-configuration-for-the-application)
  - [Configuration via fixed config file (UseCase 1)](#configuration-via-fixed-config-file)
  - [Configuration via app Configuration Service (UseCase 2)](#configuration-via-app-configuration-service)
...
- [Configuring and deploying the application to a Industrial Edge Device](#configuring-and-deploying-the-application-to-a-industrial-edge-device)
  - [Configuring the application via file upload (UseCase 1](#configuring-the-application-via-file-upload)
  - [Configuring the application via custom UI (UseCase 2)](#configuring-the-application-via-custom-ui)
...
In the example above, the application will authenticate to the IE databus with the username `edge` and password `edge`. It will subscribe to `topic1` and will publish to `topic2`.
...
If no configuration is provided (e.g if the application is deployed as a standalone application), the application will use the corresponding environmental variables specified in the `docker-compose.yml` file.

## Configuring the application via file upload (UseCase 1)
...
## Configuring the application via custom UI (UseCase 2)
...
As soon as a configuration for the application is created and the Industrial Edge Databus is configured, the application can be installed to an Industrial Edge Device.
